...
/**
 * A drop target that is neither a column nor an item — typically an insertion
 * slot registered between two cards.
 *
 * `index` is an index into the column as currently rendered, which still
 * contains the dragged item. Kanban compensates for the item's own removal, so
 * a resolver can report the slot it painted without correcting for the shift.
 */
...
> {
  ...
  modifiers?: Modifiers
  /**
   * Resolves drop targets that carry no column or item id of their own. Return
   * null to fall back to the default id-based resolution. Without this, such a
   * target resolves to no container and the drop is ignored.
   */
  resolveDropTarget?: (over: Over) => KanbanDropTarget | null
  /**
   * Overrides dnd-kit's default collision detection. Required when insertion
   * slots and cards are both registered, since the default resolves overlaps
   * by area and will favour whichever rect happens to be larger.
   */
  collisionDetection?: CollisionDetection
  /**
   * Overrides the built-in mouse/touch/keyboard sensors. Supply this when the
   * board sits among controls that need a different activation distance.
   */
...
function Kanban<T>({
  ...
  /**
   * Resolves a slot drop into concrete source/target coordinates.
   *
   * The id-based paths below swap the active card with the card it is over.
   * A slot is different: it names a position to insert *at*. Because the slot
   * index counts the column as rendered — active card included — lifting the
   * card out of a lower position shifts every slot above it down by one.
   */
  const resolveSlot = useCallback(
    (activeId: UniqueIdentifier, target: KanbanDropTarget) => {
      const sourceContainer = findContainer(activeId)
      if (!sourceContainer || !(target.container in columns)) return null

      const sourceIndex = columns[sourceContainer].findIndex(
        (item: T) => getItemValue(item) === activeId
      )
      if (sourceIndex < 0) return null

      const sameColumn = sourceContainer === target.container
      const ceiling = sameColumn
        ? columns[target.container].length - 1
        : columns[target.container].length

      let insertAt = target.index
      if (sameColumn && sourceIndex < insertAt) insertAt -= 1
      insertAt = Math.max(0, Math.min(insertAt, ceiling))

      return { sourceContainer, sourceIndex, sameColumn, insertAt }
    },
    [columns, findContainer, getItemValue]
  )

  const applySlotMove = useCallback(
    (
      activeId: UniqueIdentifier,
      target: KanbanDropTarget
    ): Record<string, T[]> | null => {
      const slot = resolveSlot(activeId, target)
      if (!slot) return null
      const { sourceContainer, sourceIndex, sameColumn, insertAt } = slot
      if (sameColumn && insertAt === sourceIndex) return null

      const nextSource = [...columns[sourceContainer]]
      const [moved] = nextSource.splice(sourceIndex, 1)
      const nextTarget = sameColumn ? nextSource : [...columns[target.container]]
      nextTarget.splice(insertAt, 0, moved)

      return sameColumn
        ? { ...columns, [sourceContainer]: nextTarget }
        : {
            ...columns,
            [sourceContainer]: nextSource,
            [target.container]: nextTarget,
          }
    },
    [columns, resolveSlot]
  )
  ...
  const handleDragOver = useCallback(
    (event: DragOverEvent) => {
      ...
      // Slot targets carry no id of their own, so they must be resolved before
      // the id-based lookup — which would find no container and bail.
      const slotTarget = resolveDropTarget?.(over) ?? null
      if (slotTarget) {
        const next = applySlotMove(active.id, slotTarget)
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      const slotTarget = isColumn(active.id)
        ? null
        : (resolveDropTarget?.(over) ?? null)

      // Handle item move callback
      if (onMove && !isColumn(active.id)) {
        const slot = slotTarget ? resolveSlot(active.id, slotTarget) : null
        const activeContainer = slot?.sourceContainer ?? findContainer(active.id)
        const overContainer = slotTarget?.container ?? findContainer(over.id)
        ...
        if (activeContainer && overContainer) {
          ...
          // A slot reports the corrected insertion index, so consumers applying
          // the move themselves get the same coordinates Kanban would have used.
          const overIndex = slot
            ? slot.insertAt
            : isColumn(over.id)
      ...
      // A slot names an insertion position rather than a card to swap with, so
      // it supersedes the id-based reorder paths below.
      if (slotTarget) {
        const next = applySlotMove(active.id, slotTarget)
        commitChange(next ?? columns, event, "item")
        if (next) setColumns(next)
        dragOriginRef.current = null
        return
      }
