 ...
 *
 * ## It shows itineraries, collections and analyzed links
 *
 * It used to merge five kinds of thing out of Supabase tables this build does
 * not have, so it resolved to an empty array on every call. Three kinds have a
 * backend now: `GET /api/itineraries`, `GET /api/collections` and
 * `GET /api/content`.
 *
 * `recent` shows all three, interleaved by date, which is what a dashboard
 * called "recent" should mean. `itinerary`, `collection` and `links` narrow to
 * one kind each. The remaining chips — locations, favourites and archived —
 * still name backends that are gone and still list nothing.
 *
 * **The three reads are independent.** One failing must not empty the grid of
 * the other kinds: a link library that cannot be read is not a reason to hide
 * somebody's trips.
 *
...
const COLLECTION_FILTERS: ReadonlySet<FilterType> = new Set<FilterType>(["recent", "collection"]);
...
export function useDashboardRecent({
  ...
  const load = useCallback(async () => {
    ...
    const wantsCollections = COLLECTION_FILTERS.has(filter);
    const wantsLinks = LINK_FILTERS.has(filter);

    if (!userId || (!wantsItineraries && !wantsCollections && !wantsLinks)) {
      setRawItems([]);
      setIsLoading(false);
      return;
    }
    ...
    const [trips, collections, links] = await Promise.allSettled([
      wantsItineraries ? getItineraries() : Promise.resolve([]),
      wantsCollections ? getCollections() : Promise.resolve([]),
    ...
    if (collections.status === "rejected") {
      console.error("[dashboard] the collection list could not be loaded", collections.reason);
    }
    ...
    const items: RecentContentItem[] = [
      ...
      ...(collections.status === "fulfilled" ? collections.value : [])
        .filter((collection) => !collection.is_archived)
        .map((collection) => ({
          id: collection.id,
          type: "collection" as const,
          name: collection.name,
          thumbnail_url: collection.thumbnail_url ?? null,
          preview_images: collection.preview_images,
          updated_at: collection.updated_at,
          is_bookmarked: collection.is_bookmarked,
